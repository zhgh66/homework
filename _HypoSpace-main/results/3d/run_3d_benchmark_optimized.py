...
class Benchmark3D:
    ...
    def create_prompt(self, observations, prior_structures=None) -> str:
        ...
        """
        # Stronger, machine-enforceable output rules to improve parser reliability
        prompt += f"""
        OUTPUT RULES (must follow exactly):
        - ONLY output the structure text and nothing else. Wrap the entire structure block in a single triple-backtick code fence (```).
        - Inside the code fence start with the word "Structure:" on its own line, then list layers from bottom to top: "Layer 1:", then rows of digits separated by single spaces.
        - Do NOT include any explanation, reasoning, notes, or extra text before, inside, or after the code fence. Any extra prose will make automated parsing fail.
        - Do NOT include unnecessary empty layers at the bottom or top. Only include layers needed to represent the structure (up to the maximum {max_height}).
        - Use spaces between digits (e.g., "1 0 1"). Do not use commas, pipes, or other separators.
        - If you cannot produce a valid structure that meets these constraints, return exactly the following inside the code fence:
        ```
        Structure:
        INVALID
        ```

        Example valid minimal response (3x3, 2 layers):
        ```
        Structure:
        Layer 1:
        1 0 1
        ...
        Follow these rules exactly. Parsers will only look inside the single code-fence for the word "Structure:" and the subsequent Layer rows.
        """
        return prompt
    ...
    def validate_structure_matches_observations(
        ...
        # Normalize structure to remove trailing empty layers and ensure consistent shape
        structure = (
            structure.normalize() if hasattr(structure, "normalize") else structure
        )

        # Enforce max_height from metadata if available
        try:
            max_height = (
                int(self.metadata.get("max_height"))
                if self.metadata and "max_height" in self.metadata
                else None
            )
        except Exception:
            max_height = None

        if max_height is not None and len(structure.layers) > max_height:
            return False

        # Require bottom layer to be non-empty (explicit check)
        if structure.layers:
            bottom_layer = structure.layers[0]
            if not np.any(bottom_layer != 0):
                return False
    ...
    def parse_llm_response(self, response: str) -> Optional[Structure3D]:
        ...
        try:
            # If the LLM wrapped the structure in a code fence, extract the fenced content first
            m = re.search(r"```(?:\w+\n)?(.*?)```", response, re.S)
            if m:
            ...
            # stronger set of reasoning indicators to stop parsing when LLM begins to explain
            ...
            for line in lines[structure_start:]:
                ...
                # If we see common explanation prefixes, stop parsing further rows
                if any(
                    line.lower().startswith(ind)
                    for ind in ["note:", "reasoning:", "explanation:"]
                ):
                    break

                # Layer header handling
                if line.startswith("Layer "):
                    match = re.match(r"Layer\s+(\d+)", line)
                    if match:
                        new_layer_num = int(match.group(1))
                        # enforce sequential numbering or allow Layer 1 start
                        if new_layer_num == layer_num + 1 or (
                            ...
                            # out-of-order layer: stop parsing further
                            break
                    continue

                # If we detect explanation lines anywhere, stop
                if any(ind in line.lower() for ind in reasoning_indicators):
                    break
                ...
                try:
                    ...
                    for x in parts:
                        if x.strip() == "":
                            continue
                        # ignore stray non-digit tokens
                        if not re.match(r"^[01]$", x.strip()):
                            # if token contains digits with other chars, try to extract digits
                            digits = re.findall(r"[01]", x)
                            if len(digits) == 1 and len(x.strip()) == 1:
                                val = int(digits[0])
                            else:
                                # not a clean binary token -> skip this token
                                continue
                        else:
                            val = int(x.strip())
            ...
            # append last collected layer
            if current_layer:
                layers.append(current_layer)

            # If no layers parsed, bail out
            if not layers:
                return None

            # Remove leading empty layers (bottom) so bottom layer is non-empty
            def layer_all_zero(layer):
                for r in layer:
                    for c in r:
                        if c != 0:
                            return False
            ...
            # Trim leading zero layers
            while layers and layer_all_zero(layers[0]):
                layers.pop(0)

            # After trimming, require at least one non-empty layer
            if not layers:
                return None

            # Enforce max_height from metadata if available
            max_height = int(self.metadata.get("max_height", 3)) if self.metadata else 3
            if len(layers) > max_height:
                # Truncate extra top layers to respect max_height
                layers = layers[:max_height]

            # Ensure bottom layer is non-zero (strict check)
            if layer_all_zero(layers[0]):
                return None
